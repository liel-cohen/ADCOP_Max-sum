 ...
 *
 */
@Register(name="adcop-meeting-scheduling-noise")
public class MeetingSchedulingADCOPNoiseGen extends AbstractSeedableProbGen {
	...
	public void __generate(Problem prob, Random rand) {
		...
		prob.initialize(ProblemType.ADCOP, domains);
		...
		for (int i = 0; i < prob.getNumberOfVariables(); i++) {
			for (int j = i+1; j < prob.getNumberOfVariables(); j++) {
				if (prob.isConstrained(i, j)) {
					splitCost(i, j, prob, rand);
				}
			}
		}
	}
